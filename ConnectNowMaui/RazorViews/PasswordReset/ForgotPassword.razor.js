...
export function validateForm(currentTab) {
    ...
    for (i = 0; i < y.length; i++) {
        ...
        if (y[i].value.trim() == "") {
            ...
            valid = false;
        } else if (y[i].type === "text" && y[i].placeholder === "Email or Phone") {
            // Validate email format
            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
            // Validate phone format (example for international numbers starting with + or local numbers)
            const phonePattern = /^\+?[1-9]\d{1,14}$/; // E.164 format or similar

            const value = y[i].value.trim(); // Trim whitespace from input
            if (!emailPattern.test(value) && !phonePattern.test(value)) {
                y[i].className += " invalid";
                valid = false;
            }
        }
            else if (y[i].type === "text" && y[i].placeholder === "Your Name") {
